...
extern int dhd_dev_get_ipaddr(struct dhd_pub *dhd);
...

{
        ...
        {
        case TRUE:
         //         printk(KERN_ERR "[WIFI] Enabling ARP Offload.\n");
#if 0
            ...
            */
            arp_filter = 0x00000009;
            if( (len = bcm_mkiovar("arp_ol", (char *)&arp_filter, 4, iovbuf, sizeof(iovbuf))) > 0 )
            {
              if(len >=100)
              {
                   printk(KERN_ERR "len value not proper!!");
                   return -1;
              }
                if((ret = dhdcdc_set_ioctl(dhd, 0, WLC_SET_VAR, iovbuf, len)) < 0)
                   printk(KERN_ERR "%s: arpfilter setting returned failure \n", __FUNCTION__);
           }
           else
           {
                   printk(KERN_ERR "%s: mkiovar failed \n", __FUNCTION__);
           }

            /*  Enable ARP Feature */
            arpoe_val = 1;
            if( (len = bcm_mkiovar("arpoe", (char *)&arpoe_val, 4, iovbuf, sizeof(iovbuf))) > 0)
            {
               if(len >=100)
               {
                    printk(KERN_ERR "len value not proper!!");
               ...
               if((ret = dhdcdc_set_ioctl(dhd, 0, WLC_SET_VAR, iovbuf, len)) < 0)
                   printk(KERN_ERR "%s: arpoe enabling returned failure ->%d \n", __FUNCTION__, ret);
            }
            else
                    printk(KERN_ERR "%s: mkiovar failed \n", __FUNCTION__);


             /*TODO: If snoop option is not working, we need to set IP using arp_hostip iovar */
             break;

case FALSE:

    //         printk(KERN_ERR "[WIFI] Diabling ARP Offload.\n");
              ...
              */
              ...
              {
                  if((ret = dhdcdc_set_ioctl(dhd, 0, WLC_SET_VAR, iovbuf, len)) < 0)
                     printk(KERN_ERR "%s: arp filter disabling returned failure \n", __FUNCTION__);
              }
              else
                    printk(KERN_ERR "%s: mkiovar failed \n", __FUNCTION__);
              /*  Disable ARP Feature */
              ...
              if( (len = bcm_mkiovar("arpoe", (char *)&arpoe_val, 4, iovbuf, sizeof(iovbuf))) > 0)
              {
                 ...
                 if((ret = dhdcdc_set_ioctl(dhd, 0, WLC_SET_VAR, iovbuf, len)) < 0)
                    printk(KERN_ERR "%s: arpoe disabling returned failure \n", __FUNCTION__);
             ...
             bcm_mkiovar("arp_hostip_clear", NULL, 0, iovbuf, sizeof(iovbuf));
             if((ret = dhdcdc_set_ioctl(dhd, 0, WLC_SET_VAR, iovbuf, sizeof(iovbuf))) < 0)
                    printk("%s: hostip clear returned failure \n", __FUNCTION__);

#endif

              /*TODO: If snoop option is not working, need to set IP using arp_hostip */
...
#if 0
/*  Function to test arp_hostip functionality.
*/
int dhd_set_hostip(struct dhd_bus * bus)
{
  char iovbuf[1024] = {0};
  dhd_pub_t *dhd = NULL;
  int ret = 0;
  int len = 0;
  uint8 ipaddr[4] = {192,168,0,198};

   if(!bus)
   {
        printk("%s: bus ptr is NULL \n",__FUNCTION__);
        return -1;
   }
   dhd = bus->dhd;

   DHD_TRACE((KERN_INFO "%s: Enter. \n", __FUNCTION__));

   if(dhd == NULL)
   {
        printk("%s: dhd ptr is NULL \n", __FUNCTION__);
        return -1;
   }

   /* clear the present hostip */
   len = bcm_mkiovar("arp_hostip_clear", NULL, 0, iovbuf, sizeof(iovbuf));
   if((ret = dhdcdc_set_ioctl(dhd, 0, WLC_SET_VAR, iovbuf, len)) < 0)
         printk("%s: hostip_clear failure \n", __FUNCTION__);

   /* Set hostip */
   if( (ret = bcm_mkiovar("arp_hostip", (char *)&ipaddr, sizeof(ipaddr), iovbuf, sizeof(iovbuf))) >  0 )
   {
        len = ret;

        if((ret = dhdcdc_set_ioctl(dhd, 0, WLC_SET_VAR, iovbuf, len)) < 0)
        {
           printk("%s: [JJ] Set filter mode failure. ret-> %d \n", __FUNCTION__, ret);
           return -1;
        }
        else
        {
          // printk("%s: [JJ] Success. returned ->%d \n", __FUNCTION__, ret);
        }

   }
   else
   {
        printk("%s: mkiovar returned 0\n",__FUNCTION__);
        return -1;
   }
   return 0;
...
int dhdsdio_set_filter(dhd_bus_t *bus, wl_filter_tag_t *filter)
{
  ...
  {
        printk("%s: Error in add filter ret=%d \n", __FUNCTION__, ret);
...
int dhd_set_pktfilter_mode(struct dhd_bus *bus, int flag)
{
 ...
 //  printk(KERN_INFO "%s: Enter. Flag->%d \n", __FUNCTION__, flag);
...
int dhd_config_pktfilter(dhd_pub_t *dhd, uint32 id ,uint32 flag)
{
   ...
   DHD_TRACE((KERN_INFO "%s: Enter.. Flag->%d \n", __FUNCTION__, flag));
   ...
   if( (len = bcm_mkiovar("pkt_filter_enable", (char *)&pktconfig, sizeof(pktconfig), iovbuf, sizeof(iovbuf))) > 0)
   {
      ...
      if((ret = dhdcdc_set_ioctl(dhd, 0, WLC_SET_VAR, iovbuf, len)) < 0)
      {
          printk("%s: [JJ] Config packetfilter failure. ret->%d \n", __FUNCTION__, ret);
          return -1;
      }
  }
  else
          printk("%s: [JJ] bcm_mkiovar failed. \n", __FUNCTION__);


   return 0;
}

#endif //BCM_HOSTWAKE && BCM_PKTFILTER
